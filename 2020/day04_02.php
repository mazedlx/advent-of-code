<?php
$passports = 'eyr:2028 iyr:2016 byr:1995 ecl:oth
...
$lines = collect(explode(PHP_EOL, $passports))
  ...
  ->map(function ($passport) {
    return collect(explode(' ', $passport->implode(' ')))->filter(function (
      $line
    ) {
      ...
      switch (true) {
        ...
        case $key === 'hgt':
          ...
          } elseif (Str::contains($value, 'in')) {
            return $value >= 59 && $value <= 76;
          }
          return false;
          break;
        case $key === 'hcl':
          return preg_match('/#[a-f|0-9]{6}/', $value) === 1;
          break;
        case $key === 'ecl':
          ...
          break;
        case $key === 'pid':
          return preg_match('/\b\d{9}\b/', $value) === 1;
          break;
        case $key === 'cid':
          return true;
          break;
  ...
  ->filter(function ($passport) {
    return $passport->count() === 8 ||
      ($passport->count() === 7 &&
        $passport
          ->filter(function ($line) {
            return Str::startsWith($line, 'cid:');
          })
          ->isEmpty());
  })
  ->count();
