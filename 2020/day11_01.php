<?php
$inputs = 'LLLLLLL.L.LL.L.LLLL.LLLLLLL.LLLLL.LLLLLL.LLLLLLL.L.LLL.LLLLLLLL.LLLLLL.LLLLLLLLLLLLLL.LLLLL
...
function adjacentOccupiedSeats($rowNum, $seatNum, $seats)
{
  $adjacents = [];
  if (
    array_key_exists($rowNum - 1, $seats) &&
    array_key_exists($seatNum - 1, $seats[$rowNum - 1])
  ) {
    $adjacents[] = $seats[$rowNum - 1][$seatNum - 1] === OCCUPIED;
  }
  if (
    array_key_exists($rowNum - 1, $seats) &&
    array_key_exists($seatNum, $seats[$rowNum - 1])
  ) {
    $adjacents[] = $seats[$rowNum - 1][$seatNum] === OCCUPIED;
  }
  if (
    array_key_exists($rowNum - 1, $seats) &&
    array_key_exists($seatNum + 1, $seats[$rowNum - 1])
  ...
  if (
    array_key_exists($rowNum, $seats) &&
    array_key_exists($seatNum - 1, $seats[$rowNum])
  ) {
    $adjacents[] = $seats[$rowNum][$seatNum - 1] === OCCUPIED;
  }
  if (
    array_key_exists($rowNum, $seats) &&
    array_key_exists($seatNum + 1, $seats[$rowNum])
  ...
  return collect($adjacents)
    ->filter()
    ->count();
}

$seats = collect(explode(PHP_EOL, $inputs))
  ->map(function ($row) {
...
while (!$found) {
  foreach ($seats as $rowNum => $row) {
    foreach ($row as $seatNum => $seat) {
      switch (true) {
        case $seat === FREE &&
          adjacentOccupiedSeats($rowNum, $seatNum, $seats) === 0:
          $changedSeats[$rowNum][$seatNum] = OCCUPIED;
          break;
        case $seat === FLOOR:
          $changedSeats[$rowNum][$seatNum] = FLOOR;
          break;
        case $seat === OCCUPIED &&
          adjacentOccupiedSeats($rowNum, $seatNum, $seats) >= 4:
          $changedSeats[$rowNum][$seatNum] = FREE;
          break;
      }
    }
  }
  $seats = $changedSeats;
  $patterns[] = collect($seats)
    ->map(function ($row) {
      return collect($row)->join('');
    })
    ->join('');

  if (
    collect($patterns)->count() !==
    collect($patterns)
      ->unique()
      ->count()
  ) {
    ...
    echo PHP_EOL;
    echo collect(str_split(collect($patterns)->last()))
