...
{
  $bagsNeeded = [];
  foreach ($search as $outerBag => $amountOuterBags) {
    foreach ($rules[$outerBag] ?? [] as $innerBag => $amountInnerBags) {
      $bagsNeeded[$innerBag] =
        ($bagsNeeded[$innerBag] ?? 0) + $amountOuterBags * $amountInnerBags;
    }
  }
  return $bagsNeeded;
}
...
while ($search) {
  $innerBags = traverseBags($search, $rules);
  $amountBagsNeeded += array_sum($innerBags);
  $search = $innerBags;
}
