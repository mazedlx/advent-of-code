...
function followWire($wire)
{
    ...
    foreach (explode(',', $wire) as $instruction) {
        ...
        $steps = (int) substr($instruction, 1, strlen($instruction) - 1);
...
$points1 = followWire($wire1);
$points2 = followWire($wire2);

$intersections = array_intersect_key($points1, $points2);
$distances = [];
foreach($intersections as $intersection => $dot) {
	$distances[] = abs(explode('/',$intersection)[0]) + abs(explode('/',$intersection)[1]);
}
sort($distances);
echo $distances[1] . PHP_EOL;