...
foreach ($aunts as $aunt) {
	preg_match_all('/Sue\s(\d+):\s|(\w+:\s\d+)/', $aunt, $matches);
	$data[$matches[1][0]] = [];
	foreach(array_filter($matches[2]) as $value) {
		$data[$matches[1][0]][explode(': ', $value)[0]] = (int) explode(': ', $value)[1];
	}
}
$hits = [];
$misses = [];
foreach($data as $id => $keys) {
	foreach($keys as $key => $value) {
		if(in_array($key, ['cats', 'trees'])) {
			if($auntSue[$key] < $data[$id][$key]) {	
				$hits[] =  $id;
			} else {
				$misses[] = $id;
			}
		} elseif(in_array($key, ['goldfish', 'pomeranians'])) {
			if($auntSue[$key] >	 $data[$id][$key]) {	
				$hits[] =  $id;
			} else {
				$misses[] = $id;
...
echo array_values(array_diff($hits, $misses))[0], PHP_EOL;