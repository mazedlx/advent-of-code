...
foreach ($rooms as $room) {
    $chars = [];
    preg_match_all('/[a-z\-]+|\d+/', $room, $matches);
    $letters = str_replace('-', '', $matches[0][0]);

    $sectorID = $matches[0][1];
    $checksum = $matches[0][2];
    $counts = count_chars($letters);
    foreach ($counts as $ascii => $count) {
        if ($count > 0 && preg_match('/\w/', chr($ascii))) {
            $chars[chr($ascii)] = $count;
        }
    }
    array_multisort(
        array_values($chars),
    ...
    if (strstr($test, $checksum)) {
        $encrypted = [];
        foreach (str_split($letters) as $key => $letter) {
            $start = ord('a');
            $end = ord('z');
            $rotate = $sectorID % 26;
            $ord = ord($letter);
            ...
            $encrypted[$key] = chr($new);
        }
        if (strstr(implode('', $encrypted), 'north')) {
            echo $sectorID, ' ', implode('', $encrypted);
        }
    }
}
