...
#$passphrases = ['abcde xyz ecdabs'];
$valid = 0;
foreach ($passphrases as $str) {
    $parts = explode(' ', $str);
    $words = count($parts);
    $unique = count(array_unique($parts));
    $sortedParts = array_map(function ($part) {
        $arr = str_split($part);
        sort($arr);
        return implode('', $arr);
    }, $parts);
    $wordsSorted = count($sortedParts);
    $uniqueSorted = count(array_unique($sortedParts));

    if ($words === $unique && $uniqueSorted === count($sortedParts)) {
        $valid += 1;
    }
}
